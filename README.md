...
Sign-in
Staff sign in with a 6-digit code emailed to them from the gym's Gmail (sent by the Apps Script bridge). Their phone stays signed in for 180 days. The app takes each person's identity from their session, not from anything they type, so one coach can't log hours or sales as another, and the raw .ondigitalocean.app link shows nothing without a code.

Signing someone out: delete or change their row on the Staff tab. They're locked out within a minute.
Signing everyone out: set or change SESSION_SECRET in DigitalOcean.
Codes not arriving: check spam first. The bridge must be redeployed as a new version after updating Code.gs (its test page should say spartan-build-3). Consumer Gmail allows about 100 emails a day, far more than sign-ins need.
iPhone home-screen icon keeps a separate sign-in from Safari: add the icon first, then sign in from the icon.
Staff and roles
