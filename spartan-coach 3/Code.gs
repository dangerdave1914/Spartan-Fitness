...
function doPost(e) {
  ...
  if (body.action === 'mail') {
    try { return json({ ok: mail(body.to, body.subject, body.text) }); }
    catch (err) { return json({ error: String(err && err.message || err) }); }
  }
  ...
  try {
    switch (body.action) {
      ...
      case 'mail':   return json({ ok: mail(body.to, body.subject, body.text) });
...
const BUILD = 'spartan-build-3';
function doGet() {
  return ContentService.createTextOutput(
    'spartan bridge ok — ' + BUILD + ' — actions: tabs, grid, grids, update, append, delete, mail');
}
...
/** Sign-in codes. Sent from the account that deployed this script. */
function mail(to, subject, text) {
  if (!/^\S+@\S+\.\S+$/.test(String(to || ''))) throw new Error('bad address');
  MailApp.sendEmail({ to: to, subject: String(subject || '').slice(0, 200), body: String(text || ''), name: 'Spartan Coach' });
  return true;
}
