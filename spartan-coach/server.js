...
app.post('/api/staff', wrap(async (req, res) => {
  ...
  if (row && row.Name) return res.json(whoami(email, o));
  // Coaches are matched to schedule slots by display name, so two people with
  // one name would get each other's classes.
  ...
  bust('ops');
  res.json(whoami(email, await ops()));
}));
...
app.post('/api/refresh', (_, res) => { cache.clear(); res.json({ ok: true }); });
